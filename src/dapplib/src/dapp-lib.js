...
export class DappLib {
	...
	static async saveString(dataString) {
		let config = DappLib.getConfig();

		const paddedMsg = dataString.padEnd(DATA_BLOB_BYTE_LENGTH);
		const buffer = Buffer.from(paddedMsg, 'utf8');

		let result = await Blockchain.put({ config }, 'greeting', buffer);
		return {
